...
class third(BaseGameplayScene):
    def __init__(self, game, player, hud, dungeon_data=None):
        # Load dungeon data if not provided
        if dungeon_data is None:
            dungeon_data_path = os.path.abspath(resource_path(os.path.join("data", "dungeons", "third.json")))
            try:
        ...
        self.maze_npc = NPC(game, npc_x, npc_y, npc_width, npc_height, (0, 255, 0), "Charlie2", "charlie_2_dialogue", sprite="graphics/player/base/gnome_m.png")
        ...
        # Complete the "Ambush the caravan" objective
        quest_tracker = QuestTracker()
        quest_tracker.update_quest_progress("ambush", "the caravan")
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Left mouse click
            if self.npc_screen_rect and self.npc_screen_rect.collidepoint(event.pos):
                ...
                for entity in self.friendly_entities:
                    if isinstance(entity, NPC) and entity.name == "Charlie2":
