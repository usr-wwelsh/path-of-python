...
class broadcast_tower(BaseGameplayScene):
    def __init__(self, game, player, hud, dungeon_data=None):
        # Load dungeon data if not provided
        if dungeon_data is None:
            dungeon_data_path = os.path.abspath(resource_path(os.path.join("data", "dungeons", "broadcast_tower.json")))
            try:
    ...
    def update(self, dt):
        ...
        if self.arrow_rect and self.player.rect.colliderect(self.arrow_rect):
            self.game.scene_manager.set_scene("tower1")
            return True
