...
class data_maw(BaseGameplayScene):
    def __init__(self, game, player, hud, dungeon_data=None):
        # Load dungeon data if not provided
        if dungeon_data is None:
            dungeon_data_path = os.path.abspath(resource_path(os.path.join("data", "dungeons", "data_maw.json")))
            try:
        ...
        self.maze_npc = NPC(game, npc_x, npc_y, npc_width, npc_height, (0, 255, 0), "maw_core", "data_maw_core_dialogue", sprite="graphics/player/base/gnome_m.png")
        ...
        quest_tracker.update_quest_progress("navigate", "the Data Maw dungeon")
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Left mouse click
            if self.npc_screen_rect and self.npc_screen_rect.collidepoint(event.pos):
                ...
                for entity in self.friendly_entities:
                    if isinstance(entity, NPC) and entity.name == "maw_core":
